function [bw,level,member]=fcmthresh(IM,sw)
...
data=reshape(IM,[],1);
[center,member,obj_fcn]=fcm(data,3);
...
lomin = min(data(label==1));
lomax = max(data(label==1));
midmin = min(data(label==2));
midmax = max(data(label==2));
himin = min(data(label==3));
himax = max(data(label==3));
 
[a,b] = imhist(IM);
figure(101);
bar(b,a);axis([0 1 0 max(a(2:end))]);hold on;
plot([lomin lomin],[0 max(a(2:end))],'r--')
plot([lomax lomax],[0 max(a(2:end))],'r--')
plot([midmin midmin],[0 max(a(2:end))],'r--')
plot([midmax midmax],[0 max(a(2:end))],'r--')
plot([himin himin],[0 max(a(2:end))],'r--')
plot([himax himax],[0 max(a(2:end))],'r--')
...
plot([level level],[0 max(a(2:end))],'g--');
